...
static int entry_mod(fat_entry_t* entry) {
    ...
    if ((entry->attributes & FAT_ATTRIBUTE_DIRECTORY) &&
        (entry->cluster < 2 || (fat->type == FAT32 && entry->cluster == fat->root_cluster))) {
        errno = EINVAL;
        return -1;
    }
    return 0;
...
static int generate_short_file_name(fat_ctxt_t* fat, uint32_t cluster, const char* long_file_name,
    ...
    for (int i = 1; i < 1000000; i++) {
        ...
        char name_buf[13];
        sprintf(name_buf, "%.*s~%s.%.*s", short_file_name_nd_len, &short_file_name[0], short_file_name_disc,
                short_file_name_ext_len, &short_file_name[8]);
        int e = errno;
...
static int find_partition(fat_ctxt_t* fat) {
    ...
    if (partition_type != 0x01 && partition_type != 0x04 && partition_type != 0x06 && partition_type != 0x0E &&
...
static int dir_insert(fat_ctxt_t* fat, uint32_t dir_cluster, const char* name, time_t create_time, int cms,
                      time_t access_time, time_t modify_time, uint8_t attributes, uint32_t cluster, uint32_t size,
                      fat_entry_t* entry) {
...
uint32_t fat_rw(fat_file_t* file, enum fat_io rw, void* buf, uint32_t byte_cnt, fat_file_t* new_file, _Bool* eof) {
    ...
    uint32_t copy_cnt = 0;
    while (byte_cnt > 0) {
        if (!no_cluster && byte_cnt >= fat->bytes_per_cluster && pos.p_cluster_sector == 0 &&
            pos.p_sector_offset == 0) {
...
int fat_rename(fat_ctxt_t* fat, fat_path_t* ent_fp, fat_path_t* dir_fp, const char* path, fat_entry_t* new_ent) {
    ...
    name[name_len] = 0;
    if (dir_insert(fat, fat_path_target(dest_fp)->cluster, name, ent->create, ent->cms, ent->access_time,
                   ent->modify_time, ent->attributes, ent->cluster, ent->size, new_ent) ||
        dir_remove(ent)) {
        goto error;
    }
exit:
