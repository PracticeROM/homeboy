...
static int32_t sdio_sendcommand(uint32_t cmd, uint32_t cmd_type, uint32_t rsp_type,
...
static bool sd0_initio(void) {
    ...
    if(!(status & SDIO_STATUS_CARD_INITIALIZED)) {
        ...
        while(tries-- > 0)
        {
            ...
            ret = sdio_sendcommand(SDIO_ACMD_SENDOPCOND, 0, SDIO_RESPONSE_R3, 0x40300000, 0, 0, NULL, &resp, sizeof(resp));
            if(ret < 0) {
                goto fail;
            }
    ...
    if(ret >= 0) {
        if((uint32_t)buffer & 0x1F) {
            ...
            while(sec_cnt > 0) {
                ...
                ret = sdio_sendcommand(SDIO_CMD_READMULTIBLOCK, SDIOCMD_TYPE_AC, SDIO_RESPONSE_R1, blk_off, secs_to_read, PAGE_SIZE512, rw_buffer, NULL, 0);
                if(ret < 0) {
                    break;
                } else {
            ...
            ret = sdio_sendcommand(SDIO_CMD_READMULTIBLOCK, SDIOCMD_TYPE_AC, SDIO_RESPONSE_R1, sector, sec_cnt, PAGE_SIZE512, buffer, NULL, 0);
        }
    ...
    if(ret >= 0) {
        if((uint32_t)buffer & 0x1F) {
            ...
            while(sec_cnt > 0) {
                ...
                ret = sdio_sendcommand(SDIO_CMD_WRITEMULTIBLOCK, SDIOCMD_TYPE_AC, SDIO_RESPONSE_R1, blk_off, secs_to_write, PAGE_SIZE512, rw_buffer, NULL, 0);
                if(ret >= 0) {
            ...
            ret = sdio_sendcommand(SDIO_CMD_WRITEMULTIBLOCK, SDIOCMD_TYPE_AC, SDIO_RESPONSE_R1, sector, sec_cnt, PAGE_SIZE512, buffer, NULL, 0);
        }
...
bool sdio_is_inserted(void) {
    return ((sdio_getstatus() & SDIO_STATUS_CARD_INSERTED) ==
            SDIO_STATUS_CARD_INSERTED);
}

bool sdio_is_initialized(void) {
    return ((sdio_getstatus() & SDIO_STATUS_CARD_INITIALIZED) ==
